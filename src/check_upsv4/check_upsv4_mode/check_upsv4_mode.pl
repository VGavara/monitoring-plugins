...
# check_upsv4_mode Nagios Plugin
#
...
use constant UPSV4_OUTPUTS => '.1.3.6.1.4.1.2254.2.4.5';
use constant UPSV4_BATTERY => '.1.3.6.1.4.1.2254.2.4.7';
...
use constant NAME    => 'check_upsv4_mode';
...
"This plugin checks working mode (online/bypass/offline with batt. normal, low or depleted)\n"
  . "on a Delta UPSv4 SNMP compliant device returning autonomy values as performance data.";
...
use constant EXAMPLE => "\n\n"
  ...
  . "It checks the working mode of a UPSv4-MIB SNMP compliant device with IP address 192.168.0.1, SNMP\n"
...
"Type check_upsv4_mode --help to get info about available working mode values.";
...
sub PerformCheck() {
    my $OID_UPSOutputStatus              = UPSV4_OUTPUTS . '.1.0';
    my $OID_UPSBatteryStatus             = UPSV4_BATTERY . '.2.0';
    my $OID_UPSEstimatedMinutesRemaining = UPSV4_BATTERY . '.5.0';
    my $OID_UPSEstimatedChargeRemaining  = UPSV4_BATTERY . '.8.0';
...
'Low threshold set but unable to get battery charge percent';
            ...
            if ( $PluginReturnValue != UNKNOWN ) {
                ...
                if ( defined $BatteryCharge && $BatteryCharge ne 'NULL' ) {
                ...
                if ( defined $BatteryBackup && $BatteryBackup ne 'NULL' ) {
