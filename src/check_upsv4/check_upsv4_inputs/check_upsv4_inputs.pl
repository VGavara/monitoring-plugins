...
# check_upsv4_inputs Nagios Plugin
#
# Checks the input levels (frequency, voltage, and/or current a)
# on a DeltaUPS-MIB DEFINITIONS
#
# Type check_upsv4_inputs --help for getting more info and examples.
#
...
use constant MIB_UPS_INPUT => '1.3.6.1.4.1.2254.2.4.4';

use constant NAME    => 'check_upsv4_inputs';
...
use constant USAGE => "Usage:\n"
  ...
  . "\t\t[-V <version>]\n";
use constant BLURB =>
  "Checks the input levels (frequency, voltage and/or current)\n"
...
use constant EXAMPLE => "\n\n"
  ...
  . "Checks the input levels of a DeltaUPS-MIB SNMP compliant device\n"
  . "with IP address 192.168.0.101.\n"
  . "Plugin returns WARNING if voltage is out of  210 to 240 volts range\n"
  . ", and returns CRITICAL if voltage is out of\n"
  . "200 to 250 volts range. In other case it returns OK.\n" . "\n"
...
sub CreateNagiosManager() {
    ...
    $Nagios->add_arg(
        spec => 'warning|w=s',
        help =>
"Warning range list with format [<frequency>],[<voltage>],[<current>]",
        required => 1
    );
    ...
    $Nagios->add_arg(
        spec => 'critical|c=s',
        help =>
"Critical range list with format [<frequency>],[<voltage>],[<current>]",
        required => 1
    );
...
sub PerformCheck() {
    ...
    my $OID_upsInputNumLines = MIB_UPS_INPUT . '.1';
    ...
    my $WarningOutput  = "";
    my $perfdata       = "";
    my @fields         = ( "Frequency", "Voltage", "Current" );
...
#($SNMPSession, $SNMPError) = Net::SNMP->session(-hostname => "192.168.6.120", -community => "public", -version => 1);
    if ( !defined($SNMPSession) ) {
        ...
        my $numLines =
          $SNMPSession->get_request( -varbindlist => [$OID_upsInputNumLines] );
        ...
        if ( !defined($numLines) ) {
            ...
            $RequestResult =
              $SNMPSession->get_entries( -columns => [MIB_UPS_INPUT] );
            if ( !defined($RequestResult) ) {
                $SNMPSession->close;
                $_[1] = "Error '$SNMPError' retrieving info ";
                return UNKNOWN;
            }
            ...
            for ( $i = 0 ; $i < $numLines ; $i++ ) {
                for ( $j = 0 ; $j < $length ; $j++ ) {
                    ...
                    $id = $i * 3 + int( $FieldsToProcess[$j] ) + 2;

#~ $RequestResult = $SNMPSession->get_request(-varbindlist => [MIB_UPS_INPUT.".$id"]);

                    #~ if (!defined($RequestResult)) {
                    #~ $_[1] = "Not exists ".MIB_UPS_INPUT.".$id";
                    #~ return UNKNOWN;
                    #~ }
                    $field = $RequestResult->{ MIB_UPS_INPUT . ".$id.0" };
                    if ( !defined $field || $field eq 'NULL' ) {
                        $_[1] =
                          "Unable to get $fields[$FieldsToProcess[$j]] value";
                        return UNKNOWN;
                    }
                    $field             = $field / 10;
                    $RequestOids[$id]  = $field;
                    $PluginReturnValue = $Nagios->check_threshold(
                        check    => $field,
                        warning  => $arrayWarningRanges[ $FieldsToProcess[$j] ],
                        critical => $arrayCriticalRanges[ $FieldsToProcess[$j] ]
                    );
                    ...
                    if ( $PluginReturnValue eq OK ) {
                        ...
                        if ( ( substr $ranges[ ( $id - 2 ) % 3 ], 0, 1 ) eq
                            "\@" )
                        {
                            my @limits = split(
                                /:/,
                                (
                                    ...
                                    1,
                                    length( $ranges[ $FieldsToProcess[$j] ] )
                                )
                            );
                            my $minMessage = "";
                            my $maxMessage = "";
                            ...
                            if ( length($minMessage) and length($maxMessage) ) {
                                $message =
                                    "Input "
                                  . ( $i + 1 )
                                  . ": $fields[$FieldsToProcess[$j]] = $field (valid range is ";
                                $message =
                                  $minMessage . " or" . $maxMessage . "); ";
                            }
                            else {
                                $message =
                                    "Input "
                                  . ( $i + 1 )
                                  . ": $fields[$FieldsToProcess[$j]] = $field (valid range is $minMessage$maxMessage); ";
                            ...
                            my $doubledot =
                              $ranges[ $FieldsToProcess[$j] ] =~ tr/://;
                            my @limits =
                              split( /:/, $ranges[ $FieldsToProcess[$j] ] );
                            my $minMessage = "";
                            my $maxMessage = "";
