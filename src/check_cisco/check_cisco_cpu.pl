...
# check_cisco_cpu Nagios Plugin
# Checks the CPU load (in percent) on a CISCO-PROCESS-MIB or OLD-CISCO-CPU-MIB
# SNMP compliant device.
...
use constant USAGE => 		"Usage:\n".
							...
							"\t\t[-V <version>]\n";
use constant BLURB => 		"This plugin checks the CPU load on a on a Cisco CISCO-PROCESS-MIB or OLD-CISCO-CPU-MIB SNMP compliant device.\n";
	...
	if (&ManageArguments($Nagios, $Error, $PluginMode)) {
		...
		if ($PluginMode == MODE_TEST) {
			...
			if ( defined $Nagios->opts->resourceid ) {
				...
				$PluginReturnValue = &PerformProcessMibCheck($Nagios, $PluginOutput);
			}
			else {
				# Checking a OLD-CISCO-CPU-MIB device
				$PluginReturnValue = &PerformOldCPUMibCheck($Nagios, $PluginOutput);
			}
		}
	}
	else {
...
sub CreateNagiosManager() {
	...
	$Nagios->add_arg(spec => 'interval|i=s',
				help => 'CPU interval: 5s (last 5 seconds, only available for CISCO-PROCESS-MIB devices), 1m (last 1 minute, default), 5m (last 5 minutes)',
				default => '1m',
				required => 0);
	
	# Add argument resource
	$Nagios->add_arg(spec => 'resourceid|r=i',
				help => 'CPU id (for CISCO-PROCESS-MIB compliant devices). Omit this argument for checking OLD-CISCO-CPU-MIB devices.',
				required => 0);

	# Add argument resource
	$Nagios->add_arg(spec => 'deprecated|d',
				help => 'Use deprecated CISCO-PROCESS-MIB cpmCPUTotal5sec, cpmCPUTotal1min and cpmCPUTotal5min OIDs.',
				required => 0);
	...
	$Nagios->add_arg(spec => 'warning|w=i',
				help => "Warning threshold",
				required => 0);
	# Add argument critical
	$Nagios->add_arg(spec => 'critical|c=i',
...
sub ManageArguments() {
	...
	# SNMP v3 auth
	if (defined($Nagios->opts->snmpuser) && defined($Nagios->opts->snmpauthprotocol) && defined($Nagios->opts->snmpauthpassword)){
		# Encryption
		if ( defined($Nagios->opts->snmpprivprotocol) && !defined($Nagios->opts->snmpprivpassword)){
			$_[1] = "SNMP error. If privace protocol is defined , privace pass phrase must be defined.";
			return 0;
		}
		elsif ( !defined($Nagios->opts->snmpprivprotocol) && defined($Nagios->opts->snmpprivpassword)){
			$_[1] = "SNMP error. If privace pass phrase is defined , privace protocol must be defined.";
			return 0;
		}
	}
	else {
		if (defined($Nagios->opts->snmpuser) || defined($Nagios->opts->snmpauthprotocol) || defined($Nagios->opts->snmpauthpassword)){	
			$_[1] = "SNMP error. If authenticacion is defined ,authentication user, authenticacion protocol and authentication pass phrase must be defined.";
			return 0;
		}		
	}
	...
	if ( $Nagios->opts->interval ne '5s' && $Nagios->opts->interval ne '1m' && $Nagios->opts->interval ne '5m' ) {
		$_[1] = 'Invalid interval value: ' . $Nagios->opts->interval .". Valid interval values are 5s (for CISCO-PROCESS-MIB devices), 1m and 5m.";
		return 0;
	}
	elsif ( $Nagios->opts->interval eq '5s' &&  !defined $Nagios->opts->resourceid ) {
		$_[1] = 'Invalid interval value: ' . $Nagios->opts->interval .". Valid interval values for OLD-CISCO-CPU-MIB devices are 1m and 5m.";
		return 0;	 	 
	}
	...
	if ( defined($Nagios->opts->warning) && defined($Nagios->opts->critical) ) {
		...
		if ( !defined($Nagios->opts->warning) && !defined($Nagios->opts->critical) ){
			...
			$_[1] = "Invalid argument set. You must define, at least, warning and critical arguments in check mode or none of these in test mode.";
			return 0;		
		}
	}
...
sub TestHost() {
	...
	if (defined($SNMPSession)) {
		...
		$RequestResult = $SNMPSession->get_request(-varbindlist => [$OID_occAvgBusy1, $OID_occAvgBusy5]);
		if ( defined $RequestResult && ($RequestResult->{$OID_occAvgBusy1} ne NOSUCHOBJECT || $RequestResult->{$OID_occAvgBusy5} ne NOSUCHOBJECT) ) {
			$Output = "CISCO CPU DATA (from OLD_CISCO_CPU_MIB)\n" .
			'CPU avgBusy1: ' . ($RequestResult->{$OID_occAvgBusy1} eq NOSUCHOBJECT ? '-' : $RequestResult->{$OID_occAvgBusy1}) . "\t" .
			'CPU avgBusy5: ' . ($RequestResult->{$OID_occAvgBusy5} eq NOSUCHOBJECT ? '-' : $RequestResult->{$OID_occAvgBusy5}) . "\n";			
		}
		else {
			...
			$RequestResult = $SNMPSession->get_entries(-columns => [$OID_cpmCPUTotalEntry]);
			$RequestResultIndex = $SNMPSession->get_entries(-columns => [$OID_cpmCPUTotalPhysicalIndex]);
			
			$RequestResultDesc=$SNMPSession->get_entries(-columns => [CISCO_ENTPHYSICALDESCR]);
			...
			foreach $Oid (keys %{$RequestResultIndex}){
				...
				$desc = $RequestResultDesc->{CISCO_ENTPHYSICALDESCR.".$entId"} if defined $RequestResultDesc->{CISCO_ENTPHYSICALDESCR.".$entId"};
				$Output .=  "CPU id: $id\t" .
							"CPU description: $desc\t";
							
				if ( defined $RequestResult->{$OID_cpmCPUTotal5secRev.$id} && $RequestResult->{$OID_cpmCPUTotal1minRev.$id} && $RequestResult->{$OID_cpmCPUTotal5minRev.$id} ) {
					$Output .= "CPU Load5sec: ".$RequestResult->{$OID_cpmCPUTotal5secRev.$id}."\t" .
							   "CPU Load1min: ".$RequestResult->{$OID_cpmCPUTotal1minRev.$id}."\t" .
							   "CPU Load5min: ".$RequestResult->{$OID_cpmCPUTotal5minRev.$id}."\n";
				}
				elsif ( defined $RequestResult->{$OID_cpmCPUTotal5sec.$id} && $RequestResult->{$OID_cpmCPUTotal1min.$id} && $RequestResult->{$OID_cpmCPUTotal5min.$id} ) {
					$Output .= "CPU Load5sec: ".$RequestResult->{$OID_cpmCPUTotal5sec.$id}."\t" .
							   "CPU Load1min: ".$RequestResult->{$OID_cpmCPUTotal1min.$id}."\t" .
							   "CPU Load5min: ".$RequestResult->{$OID_cpmCPUTotal5min.$id}."\t" .
...
sub PerformOldCPUMibCheck() {
	...
	if (defined($SNMPSession)) {
 		...
 		if ($Nagios->opts->interval eq '1m') {
 			$RequestResult = $SNMPSession->get_request(-varbindlist => [$OID_occAvgBusy1]);
 			$value = $RequestResult->{$OID_occAvgBusy1};
 			$Interval = 'Last-minute';
			$Variable = 'avgBusy1';
 		}
		elsif ($Nagios->opts->interval eq '5m') {
 			$RequestResult = $SNMPSession->get_request(-varbindlist => [$OID_occAvgBusy5]);
		...
		if ( defined $RequestResult ) {
			...
			if ( !defined $value ) {
				$PluginOutput = "CPU interval ($Nagios->{opts}->{interval}) not defined";
			}
			else {
				$PluginReturnValue = $Nagios->check_threshold(check => $value, warning => $Nagios->opts->warning,critical => $Nagios->opts->critical);
	
				$PluginOutput = "$Interval average of CPU busy percentage: $value%";
				if ( $PluginReturnValue == CRITICAL ) {
					$PluginOutput .= " (Critical threshold is $Nagios->{opts}->{critical}%)";
				}
				elsif ( $PluginReturnValue == WARNING ) {
					$PluginOutput .= " (Warning threshold is $Nagios->{opts}->{warning}%)";
				}
	
				#Set performance data
				$PerformanceData = "$Variable=$value%;$Nagios->{opts}->{warning};$Nagios->{opts}->{critical};0;100";
...
sub PerformProcessMibCheck() {
	...
	if (defined($SNMPSession)) {
 		# Get data
 		push (@RequestItems, "$OID_cpmCPUTotalPhysicalIndex.$Nagios->{opts}->{resourceid}");
 		if ($Nagios->opts->interval eq '5s') {
 			defined $Nagios->opts->{deprecated} ? push (@RequestItems, $OID_cpmCPUTotal5sec.$Nagios->opts->resourceid) : push (@RequestItems, $OID_cpmCPUTotal5secRev.$Nagios->opts->resourceid);	
 			$Interval = "Last-5-seconds";
			$Variable = "Load5sec";
 		}
		elsif ($Nagios->opts->interval eq '1m') {
 			defined $Nagios->opts->{deprecated} ? push (@RequestItems, $OID_cpmCPUTotal1min.$Nagios->opts->resourceid) : push (@RequestItems, $OID_cpmCPUTotal1minRev.$Nagios->opts->resourceid);
 			$Interval = "Last-1-minute";
			$Variable = "Load1min";
 		}
		elsif ($Nagios->opts->interval eq '5m') {
			defined $Nagios->opts->{deprecated} ? push (@RequestItems, $OID_cpmCPUTotal5min.$Nagios->opts->resourceid) : push (@RequestItems, $OID_cpmCPUTotal5minRev.$Nagios->opts->resourceid);
 			$Interval = "Last-5-minutes";
			$Variable = "Load5min";
 		}
		...
		if (defined ($RequestResult = $SNMPSession->get_request(-varbindlist => \@RequestItems))) {
			# Check thresholds and set plugin output
			my $RequestResultDesc = $SNMPSession->get_entries(-columns => [CISCO_ENTPHYSICALDESCR]);
			
			my $desc = $RequestResultDesc->{CISCO_ENTPHYSICALDESCR . '.' . $RequestResult->{"$OID_cpmCPUTotalPhysicalIndex.$Nagios->{opts}->{resourceid}"}};
			...
			if ($value eq NOSUCHINSTANCE) {
				$SNMPSession->close;
				$_[1] = "CPU with id ".$Nagios->opts->resourceid." doesn't exist.";
				return $PluginReturnValue;
			}
			elsif ($value eq NOSUCHOBJECT) {
				$SNMPSession->close;
				$_[1] = "CPU data not available. Maybe deprecated mode doesn't matching with device?";
				return $PluginReturnValue;
			}
			...
			if ( $PluginReturnValue == CRITICAL ) {
				$PluginOutput .= " ($value%) is above $Nagios->{opts}->{critical}%.";
			}
			elsif ( $PluginReturnValue == WARNING ) {
				$PluginOutput .= " ($value%) is above $Nagios->{opts}->{warning}%.";
			}
			else {
				$PluginOutput .= " = $value%";
			}
