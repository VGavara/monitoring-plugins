...
# check_cisco_memory Nagios-compatible plugin
...
use constant USAGE => "Usage:\n"
  ...
  . "\t\t[-V <version>]\n";
use constant BLURB =>
"This plugin checks the memory usage on a  CISCO_MEMORY_POOL_MIB SNMP compliant device.\n";
...
sub CreateNagiosManager() {
    ...
    $Nagios->add_arg(
        spec => 'warning|w=s',
        help =>
          "Warning range list with format <memory usage>,<fragmented memory>",
        required => 0
    );
    ...
    $Nagios->add_arg(
        spec => 'critical|c=s',
        help =>
          "Critical range list with format <memory usage>,<fragmented memory>",
        required => 0
    );
...
sub ManageArguments() {
        ...
        && ( defined( $Nagios->opts->critical ) ) )
    {
        ...
        if ( $Nagios->opts->warning !~ /^\d+,\d+$/ ) {
            $_[1] =
"Invalid warning threshold list: must be a comma separated used and fragmented memory thresholds.";
            return 0;
        }
        ...
        if ( $Nagios->opts->critical !~ /^(\d+,)*\d+$/ ) {
            $_[1] =
"Invalid critical threshold list: must be a comma separated used and fragmented memory thresholds.";
            return 0;
        }
    }
    else {
...
sub TestHost() {
    ...
    my $OID_ciscoMemoryPoolUsed =
      CISCO_MEMORY_POOL_MIB . CISCO_MEMORY_POOL_ENTRY . CISCO_MEMORY_POOL_USED;
    my $OID_ciscoMemoryPoolFree =
      CISCO_MEMORY_POOL_MIB . CISCO_MEMORY_POOL_ENTRY . CISCO_MEMORY_POOL_FREE;
    my $OID_ciscoMemoryPoolName =
      CISCO_MEMORY_POOL_MIB . CISCO_MEMORY_POOL_ENTRY . CISCO_MEMORY_POOL_NAME;
    my $OID_ciscoMemoryPoolLargestFree =
        CISCO_MEMORY_POOL_MIB
      . CISCO_MEMORY_POOL_ENTRY
...
#~ ($SNMPSession, $SNMPError) = Net::SNMP->session(-hostname => $Nagios->opts->hostname, -community => $Nagios->opts->community, -version => $Nagios->opts->snmpver, -port => $Nagios->opts->port, -timeout => $Nagios->opts->timeout);
    ...
    if ( defined($SNMPSession) ) {
        my $RequestResultIndex =
          $SNMPSession->get_entries( -columns => [$OID_ciscoMemoryPoolName] );
        my $RequestResult = $SNMPSession->get_entries(
            -columns => [ CISCO_MEMORY_POOL_MIB . CISCO_MEMORY_POOL_ENTRY ] );
        if ( !defined($RequestResult) && !defined($RequestResultIndex) ) {
            ...
            if ( defined($SNMPError) && ( $SNMPError ne '' ) ) {
                ...
                $_[1] =
"Empty data set recovered. Probably device didn't support CISCO-MEMORY_POOL_MIB";
            }
            $SNMPSession->close();
            return UNKNOWN;
        }
        ...
        foreach $Oid ( keys %{$RequestResultIndex} ) {
            ...
            $Output .=
              ...
              . "Memory free: "
              . $RequestResult->{ $OID_ciscoMemoryPoolFree . ".$id" } . "\t"
              . "Memory Used: "
              . $RequestResult->{ $OID_ciscoMemoryPoolUsed . ".$id" } . "\t"
              . "Memory largest free block = "
              . $RequestResult->{ $OID_ciscoMemoryPoolLargestFree . ".$id" }
              . "\n";
        }
...
sub PerformCheck() {
    ...
    if ( defined($SNMPSession) ) {
        ...
        {
            if (   $RequestResult->{$OID_ciscoMemoryPoolFree} =~ /^\d+$/
                && $RequestResult->{$OID_ciscoMemoryPoolUsed} =~ /^\d+$/ )
            {
                ...
                my $MemoryUsagePercent =
                  ($MemoryUsage) * 100 / ( $MemoryFree + $MemoryUsage );
                ...
                $PluginReturnValue = $Nagios->check_threshold(
                    ...
                    warning  => $arrayWarning[0],
                    critical => $arrayCritical[0]
                );

                my $description =
                  "'$RequestResult->{$OID_ciscoMemoryPoolName}'";
                if ( $PluginReturnValue == CRITICAL ) {
                    $CriticalOutput .= sprintf(
                        $description . ' memory usage (%.1f%%) is above %d%%.',
                        $MemoryUsagePercent, $arrayCritical[0] );
                }
                elsif ( $PluginReturnValue == WARNING ) {
                    $WarningOutput .= sprintf(
                        $description . ' memory usage (%.1f%%) is above %d%%.',
                        $MemoryUsagePercent, $arrayWarning[0] );
                }
                else {
                    $OkOutput .=
                      sprintf( $description
                          . ' memory usage = %.1f%% (%d of %d bytes)',
                        $MemoryUsagePercent, $MemoryUsage,
                        $MemoryFree + $MemoryUsage );
                }

                #Set used memory performance data
                $PerformanceData = sprintf( 'MemUsed=%.1f%%;%d;%d;0;100',
                    $MemoryUsagePercent, $arrayWarning[0], $arrayCritical[0] );

                # Fragmented memory
                my $MemoryPoolLargestFree =
                  $RequestResult->{$OID_ciscoMemoryPoolLargestFree};

#my $MemoryFragmentedPercent = (1- (($MemoryPoolLargestFree) / ($MemoryFree+$MemoryUsage)))*100;
                my $MemoryFragmentedPercent =
                  ( 1 - ( $MemoryPoolLargestFree / $MemoryFree ) ) * 100;

                $PluginReturnValue = $Nagios->check_threshold(
                    check    => $MemoryFragmentedPercent,
                    warning  => $arrayWarning[1],
                    critical => $arrayCritical[1]
                );
                if ( $PluginReturnValue == CRITICAL ) {
                    ...
                    $CriticalOutput .= sprintf(
                        ' fragmented memory flag (%.1f%%) is above %d%%.',
                        $MemoryFragmentedPercent, $arrayCritical[1] );
                }
                elsif ( $PluginReturnValue == WARNING ) {
                    if ( $WarningOutput ne '' ) {
                        $WarningOutput .= ' and';
                    ...
                    $WarningOutput .= sprintf(
                        ' fragmented memory flag (%.1f%%) is above %d%%.',
                        $MemoryFragmentedPercent, $arrayWarning[1] );
                }
                else {
                    if ( $OkOutput ne '' ) {
                        $OkOutput .= ' and';
                    ...
                    $OkOutput .= sprintf(
' fragmented memory flag = %.1f%% (Largest block free %d bytes)',
                        $MemoryFragmentedPercent, $MemoryPoolLargestFree );
                }

                #Set fragmented memory flag performance data
                $PerformanceData .=
                  sprintf( ' Fragmentation=%.1f%%;%d;%d;0;100',
                    $MemoryFragmentedPercent, $arrayWarning[1],
                    $arrayCritical[1] );
                ...
                $PluginOutput =
                  "No data found for pool '$Nagios->{opts}->{poolid}'";
            }
        }
        else {
            $SNMPError = $SNMPSession->error();
