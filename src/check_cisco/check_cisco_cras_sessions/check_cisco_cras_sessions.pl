...
# check_cisco_cras_sessions Nagios-compatible plugin
#
...
use constant SESSION_SHORT_NAMES => {
    CRASEMAIL => 'email',
    CRASIPSEC => 'ipsec',
    CRASL2L   => 'l2l',
    CRASLB    => 'lb',
...
"Usage:\ncheck_cisco_cras_sessions -H <hostname> [-w <warning threshold>] [-c <critical threshold>]\n"
  ...
  . "\t\t[-V <version>]\n";
use constant BLURB =>
"This plugin checks the number of active sessions on a Cisco Remote Access Server\n"
  . "supporting the ciscoRemoteAccessMonitorMIB MIB.";
...
sub createNagiosManager() {
    ...
    $nagios->add_arg(
        spec => 'session|s=s@',
        help =>
"Session type to check: email, ipsec, l2l (LAN to LAN), lb (load balancing), svc (SSL VPN client), webvpn. Can be defined multiple times",
        required => 0
    );
    ...
    $nagios->add_arg(
        spec => 'total|T',
        help =>
"Check is performed taking the sum of all sessions instead of every one",
        required => 0
    );
    ...
    $nagios->add_arg(
        spec => 'percent|p',
        help =>
"Check is performed taking the percent of all sesions based on device max supportable sessions",
        required => 0
    );
    ...
    $nagios->add_arg(
        spec => 'warning|w=s',
        help =>
"Warning threshold. Applies to any session type except if using the -t argument. In this case applies to the total of all sessions",
        required => 0
    );
    ...
    $nagios->add_arg(
        spec => 'critical|c=s',
        help =>
"Critical threshold. Applies to any session type except if using the -t argument. In this case applies to the total of all sessions",
        required => 0
    );
...
sub performCheck() {
    ...
    if ( !defined($SNMPSession) ) {
        ...
        # Go for them
        if ( my $requestResult =
            $SNMPSession->get_request( -varbindlist => \@OIDs ) )
        {
            ...
            if ( defined $nagios->opts->total ) {
                ...
                if ( defined $requestResult->{&OID_CRASEMAILNUM}
                    && $requestResult->{&OID_CRASEMAILNUM} > $sessions )
                {
                    $sessions    = $requestResult->{&OID_CRASEMAILNUM};
                    $sessionName = &SESSION_NAMES->{CRASEMAIL};
                }
                if ( defined $requestResult->{&OID_CRASIPSECNUM}
                    && $requestResult->{&OID_CRASIPSECNUM} > $sessions )
                {
                    $sessions    = $requestResult->{&OID_CRASIPSECNUM};
                    $sessionName = &SESSION_NAMES->{CRASIPSEC};
                }
                if ( defined $requestResult->{&OID_CRASL2LNUM}
                    && $requestResult->{&OID_CRASL2LNUM} > $sessions )
                {
                    $sessions    = $requestResult->{&OID_CRASL2LNUM};
                    $sessionName = &SESSION_NAMES->{CRASL2L};
                }
                if ( defined $requestResult->{&OID_CRASLBNUM}
                    && $requestResult->{&OID_CRASLBNUM} > $sessions )
                {
                    $sessions    = $requestResult->{&OID_CRASLBNUM};
                    $sessionName = &SESSION_NAMES->{CRASLB};
                }
                if ( defined $requestResult->{&OID_CRASSVCNUM}
                    && $requestResult->{&OID_CRASSVCNUM} > $sessions )
                {
                    $sessions    = $requestResult->{&OID_CRASSVCNUM};
                    $sessionName = &SESSION_NAMES->{CRASSVC};
                }
                if ( defined $requestResult->{&OID_CRASWEBVPNNUM}
                    && $requestResult->{&OID_CRASWEBVPNNUM} > $sessions )
                {
            ...
            if ( defined $nagios->opts->percent ) {
                ...
                $sessions = $sessions /
                  $requestResult->{&OID_MAXSESSIONSUPPORTABLE} * 100;
            }
            ...
            if ( $pluginReturnValue == OK ) {
                $pluginOutput =
                  'All session counters are in the expected range';
            }
            else {
                if ( defined $nagios->opts->total ) {
                    $pluginOutput = sprintf(
'Total session counters (%i%s) are out of the expected range',
                        $sessions,
                        (
                            defined( $nagios->opts->percent )
                            ? '% over the device max supportable sessions'
                            : ' sessions'
                        )
                    );
                }
                    ...
                    $pluginOutput = sprintf(
                        $sessionName
                          . ' session counters (%i%s) are out of the expected range',
                        $sessions,
                        (
                            defined( $nagios->opts->percent )
                            ? '% over the device max supportable sessions'
            ...
            if ( defined $nagios->opts->total ) {
                $nagios->add_perfdata(
                    ...
                    value => $sessions,
                    uom   => (
                        defined( $nagios->opts->percent ) ? '%' : 'sessions'
                    )
                );
            }
            else {
                $nagios->add_perfdata(
                    label => &SESSION_NAMES->{CRASEMAIL},
                    value => (
                          $nagios->opts->percent
                        ? $requestResult->{&OID_CRASEMAILNUM} /
                          $requestResult->{&OID_MAXSESSIONSUPPORTABLE} * 100
                        : $requestResult->{&OID_CRASEMAILNUM}
                    ),
                    uom => (
                ...
                $nagios->add_perfdata(
                    label => &SESSION_NAMES->{CRASIPSEC},
                    value => (
                          $nagios->opts->percent
                        ? $requestResult->{&OID_CRASIPSECNUM} /
                          $requestResult->{&OID_MAXSESSIONSUPPORTABLE} * 100
                        : $requestResult->{&OID_CRASIPSECNUM}
                    ),
                    uom => (
                ...
                $nagios->add_perfdata(
                    label => &SESSION_NAMES->{CRASL2L},
                    value => (
                          $nagios->opts->percent
                        ? $requestResult->{&OID_CRASL2LNUM} /
                          $requestResult->{&OID_MAXSESSIONSUPPORTABLE} * 100
                        : $requestResult->{&OID_CRASL2LNUM}
                    ),
                    uom => (
                ...
                $nagios->add_perfdata(
                    label => &SESSION_NAMES->{CRASLB},
                    value => (
                          $nagios->opts->percent
                        ? $requestResult->{&OID_CRASLBNUM} /
                          $requestResult->{&OID_MAXSESSIONSUPPORTABLE} * 100
                        : $requestResult->{&OID_CRASLBNUM}
                    ),
                    uom => (
                ...
                $nagios->add_perfdata(
                    label => &SESSION_NAMES->{CRASSVC},
                    value => (
                          $nagios->opts->percent
                        ? $requestResult->{&OID_CRASSVCNUM} /
                          $requestResult->{&OID_MAXSESSIONSUPPORTABLE} * 100
                        : $requestResult->{&OID_CRASSVCNUM}
                    ),
                    uom => (
                ...
                $nagios->add_perfdata(
                    label => &SESSION_NAMES->{CRASWEB},
                    value => (
                          $nagios->opts->percent
                        ? $requestResult->{&OID_CRASWEBVPNNUM} /
                          $requestResult->{&OID_MAXSESSIONSUPPORTABLE} * 100
                        : $requestResult->{&OID_CRASWEBVPNNUM}
                    ),
                    uom => (
            ...
            $pluginOutput =
              'Error \'' . $SNMPSession->error() . '\' getting SNMP data';
        }
