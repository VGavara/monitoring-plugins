...
use constant USAGE => 	"Usage:\n".
						"check_cisco_envmon -H <hostname>\n" .
							...
							"\t\t[-V <version>]\n";
use constant BLURB => 	"This plugin checks the enviroment sensors on a CISCO-ENVMON-MIB compliant device.";
...
sub CreateNagiosManager() {
	...
	$Nagios->add_arg(spec => 'voltage|g=s',
				help => 'Comma separated voltage id list or \'all\' (all voltage ids present)',
				required => 0);
	
	# Add argument temperature
	$Nagios->add_arg(spec => 'temperature|T=s',
				help => 'Comma separated temperature id list or \'all\' (all temperature ids present)',
				required => 0);				
		
	# Add argument fan
	$Nagios->add_arg(spec => 'fan|f=s',
				help => 'Comma separated fan id list or \'all\' (all fan ids present)',
				required => 0);
	
	# Add argument supply
	$Nagios->add_arg(spec => 'supply|s=s',
				help => 'Comma separated supply id list or \'all\' (all supply ids present)',
				required => 0);				
	...
	if ( (defined($Nagios->opts->warning)) && (defined($Nagios->opts->critical))) {
		...
		if (defined ($Nagios->opts->voltage)) {
			if ( $Nagios->opts->voltage !~ /^((\d+,)*\d+)|all$/) {
				$_[1] = "Invalid voltage number list: must be a comma separated voltage-id list or 'all'";
				return 0;
			}	
		}
		...
		if (defined ($Nagios->opts->temperature)) {
			if ( $Nagios->opts->temperature !~ /^((\d+,)*\d+)|all$/) {
				$_[1] = "Invalid temperature number list: must be a comma separated temperature-id list or 'all'";
				return 0;
			}
		}
		...
		if (defined ($Nagios->opts->fan)) {
			if ( $Nagios->opts->fan !~ /^((\d+,)*\d+)|all$/) {
				$_[1] = "Invalid fan number list: must be a comma separated fan-id list or 'all'";
				return 0;
			}
		}
		# Check supply list
		if (defined ($Nagios->opts->supply)) {
			if ( $Nagios->opts->supply !~ /^((\d+,)*\d+)|all$/) {
				$_[1] = "Invalid supply number list: must be a comma separated supply-id list or 'all'";
				return 0;
			}	
		}
		...
		if ( $Nagios->opts->warning !~ /^(\d+,)*\d+$/) {
			$_[1] = "Invalid warning threshold list: must be a comma separated environmental cisco state ids";
			return 0;
		}
	
		# Check critical value list
		if ( $Nagios->opts->critical !~/^(\d+,)*\d+$/) {
			$_[1] = "Invalid critical threshold list: must be a comma separated environmental cisco state ids";
			return 0;
		}
	...
	if (defined($SNMPSession)) {
		...
		if (defined $RequestResult) {
			...
			foreach (@VoltageIdsPresent) {
				...
				if (defined($RequestResult->{$OID_EnvMonVoltageStatusEntry.".1.".$VoltageId})) {
					# index defined
					$Output .= "Index: ".$RequestResult->{$OID_EnvMonVoltageStatusEntry.".1.".$VoltageId}."\t";
				}
				if (defined($RequestResult->{$OID_EnvMonVoltageStatusEntry.".2.".$VoltageId})) {
					# description defined
					$Output .= "Description: ".$RequestResult->{$OID_EnvMonVoltageStatusEntry.".2.".$VoltageId}."\t";
				}
				if (defined($RequestResult->{$OID_EnvMonVoltageStatusEntry.".3.".$VoltageId})) {
					# value defined
					$Output .= "Value: ".$RequestResult->{$OID_EnvMonVoltageStatusEntry.".3.".$VoltageId}."mV\t";
				}
				if (defined($RequestResult->{$OID_EnvMonVoltageStatusEntry.".4.".$VoltageId})) {
					# thresholdlow defined
					$Output .= "Threshold Low: ".$RequestResult->{$OID_EnvMonVoltageStatusEntry.".4.".$VoltageId}."\t";
				}
				if (defined($RequestResult->{$OID_EnvMonVoltageStatusEntry.".5.".$VoltageId})) {
					# thresholdlow defined
					$Output .= "Description: ".$RequestResult->{$OID_EnvMonVoltageStatusEntry.".5.".$VoltageId}."\t";
				}
				if (defined($RequestResult->{$OID_EnvMonVoltageStatusEntry.".6.".$VoltageId})) {
					# last shutdown defined
					$Output .= 'Last shutdown: ' . ENVMON_STATES->[$RequestResult->{"$OID_EnvMonVoltageStatusEntry.6.$VoltageId"}] . ' (' . $RequestResult->{"$OID_EnvMonVoltageStatusEntry.6.$VoltageId"} . ")\t";
				}
				if (defined($RequestResult->{$OID_EnvMonVoltageStatusEntry.".7.".$VoltageId})) {
					# state defined
					$Output .= 'State: ' . $RequestResult->{"$OID_EnvMonVoltageStatusEntry.7.$VoltageId"} . 
								' (' . ENVMON_STATES->[$RequestResult->{"$OID_EnvMonVoltageStatusEntry.7.$VoltageId"}] . ': ' . ENVMON_STATE_DESC->[$RequestResult->{"$OID_EnvMonVoltageStatusEntry.7.$VoltageId"}] . ")\t";
				}
			}
			...
			foreach (@TemperatureIdsPresent) {
				...
				if (defined($RequestResult->{$OID_EnvMonTemperatureStatusEntry.".1.".$TemperatureId})) {
					# index defined
					$Output .= "Index: ".$RequestResult->{$OID_EnvMonTemperatureStatusEntry.".1.".$TemperatureId}."\t";
				}
				if (defined($RequestResult->{$OID_EnvMonTemperatureStatusEntry.".2.".$TemperatureId})) {
					# description defined
					$Output .= "Description: ".$RequestResult->{$OID_EnvMonTemperatureStatusEntry.".2.".$TemperatureId}."\t";
				}
				if (defined($RequestResult->{$OID_EnvMonTemperatureStatusEntry.".3.".$TemperatureId})) {
					# value defined
					$Output .= "Value: ".$RequestResult->{$OID_EnvMonTemperatureStatusEntry.".3.".$TemperatureId}."C\t";
				}
				if (defined($RequestResult->{$OID_EnvMonTemperatureStatusEntry.".4.".$TemperatureId})) {
					# threshold defined
					$Output .= "Threshold: ".$RequestResult->{$OID_EnvMonTemperatureStatusEntry.".4.".$TemperatureId}."\t";
				}
				if (defined($RequestResult->{$OID_EnvMonTemperatureStatusEntry.".5.".$TemperatureId})) {
					# last shutdown defined
					$Output .= "Last Shutdown: ".$RequestResult->{$OID_EnvMonTemperatureStatusEntry.".5.".$TemperatureId}."\t";
				}
				if (defined($RequestResult->{$OID_EnvMonTemperatureStatusEntry.".6.".$TemperatureId})) {
					# state defined
					$Output .= 'State: ' . $RequestResult->{"$OID_EnvMonTemperatureStatusEntry.6.$TemperatureId"} . 
								' (' . ENVMON_STATES->[$RequestResult->{"$OID_EnvMonTemperatureStatusEntry.6.$TemperatureId"}] . ': ' . ENVMON_STATE_DESC->[$RequestResult->{"$OID_EnvMonTemperatureStatusEntry.6.$TemperatureId"}] . ")\t";					
				}
			}
			if ($#FanIdsPresent>=0) {
				$Output .= "\nEnvMonFanStatus";
			...
			foreach (@FanIdsPresent) {
				my $FanId= $_;
				$Output .= "\nFan Id: $FanId\t";
				if (defined($RequestResult->{$OID_EnvMonFanStatusEntry.".1.".$FanId})) {
					# index defined
					$Output .= "Index: ".$RequestResult->{$OID_EnvMonFanStatusEntry.".1.".$FanId}."\t";
				}
				if (defined($RequestResult->{$OID_EnvMonFanStatusEntry.".2.".$FanId})) {
					# description defined
					$Output .= "Description: ".$RequestResult->{$OID_EnvMonFanStatusEntry.".2.".$FanId}."\t";
				}
				if (defined($RequestResult->{$OID_EnvMonFanStatusEntry.".3.".$FanId})) {
					# state defined
					$Output .= 'State: ' . $RequestResult->{"$OID_EnvMonFanStatusEntry.3.$FanId"} . 
								' (' . ENVMON_STATES->[$RequestResult->{"$OID_EnvMonFanStatusEntry.3.$FanId"}] . ': ' . ENVMON_STATE_DESC->[$RequestResult->{"$OID_EnvMonFanStatusEntry.3.$FanId"}] . ")\t";
				}
			}
			...
			foreach (@SupplyIdsPresent) {
				my $SupplyId= $_;
				$Output .= "\nSupply Id: $SupplyId\t";
				if (defined($RequestResult->{$OID_EnvMonSupplyStatusEntry.".1.".$SupplyId})) {
					# index defined
					$Output .= "Index: ".$RequestResult->{$OID_EnvMonSupplyStatusEntry.".1.".$SupplyId}."\t";
				}
				if (defined($RequestResult->{$OID_EnvMonSupplyStatusEntry.".2.".$SupplyId})) {
					# description defined
					$Output .= "Description: ".$RequestResult->{$OID_EnvMonSupplyStatusEntry.".2.".$SupplyId}."\t";
				}
				if (defined($RequestResult->{$OID_EnvMonSupplyStatusEntry.".3.".$SupplyId})) {
					# state defined
					$Output .= 'State: ' . $RequestResult->{"$OID_EnvMonSupplyStatusEntry.3.$SupplyId"} . 
								' (' . ENVMON_STATES->[$RequestResult->{"$OID_EnvMonSupplyStatusEntry.3.$SupplyId"}] . ': ' . ENVMON_STATE_DESC->[$RequestResult->{"$OID_EnvMonSupplyStatusEntry.3.$SupplyId"}] . ")\t";
				}
			}
			...
			$_[1] = "Error '$SNMPError' requesting envmon data ".
...
sub PerformCheck() {
	...
	if (defined($SNMPSession)) {
		...
		if ( (!defined($Nagios->opts->voltage)) && (!defined($Nagios->opts->temperature)) && (!defined($Nagios->opts->fan)) && (!defined($Nagios->opts->supply)) ) {
			$voltage='all';
			$temperature='all';
			$fan='all';
			$supply='all';
		...
		if (defined($voltage)) {
			...
			if ($voltage eq 'all') { #All voltages are checked
				...
				if ($#VoltageIds eq -1) { #no voltages present
					if ($SomeEnvironmentDefined eq 1) { #Environment argument defined by user
						$SNMPSession->close();
			...
			foreach (@VoltageIds) {
				my $VoltageId= $_;
				my $VoltageState=$RequestResult->{$OID_EnvMonVoltageStatusEntry.".7.".$VoltageId};
				...
				my $VoltageDesc;
				if (defined($RequestResult->{$OID_EnvMonVoltageStatusEntry.".2.".$VoltageId})) {#description defined
					$VoltageDesc = $RequestResult->{$OID_EnvMonVoltageStatusEntry.".2.".$VoltageId};							
				}
				else {
					$SNMPSession->close();
					$_[1]="Error recovering voltage description with id $VoltageId";
					return UNKNOWN;
				}
				for (my $i=0; $i <= $#CriticalStates; $i++) {
					if ($VoltageState eq $CriticalStates[$i]) {
						$CriticalOutput .= "Sensor '$VoltageDesc'";
						if ($RequestResult->{$OID_EnvMonVoltageStatusEntry.".3.".$VoltageId}) {#value defined
							$CriticalOutput .= "with value = ".$RequestResult->{$OID_EnvMonVoltageStatusEntry.".3.".$VoltageId}."mV";
						}
						$CriticalOutput .= ' is in ' . ENVMON_STATES->[$VoltageState] . ' state (' . ENVMON_STATE_DESC->[$VoltageState] . '); ';
						last;
					}
				}
				if ($CriticalOutput eq '') {
					for (my $i=0; $i <= $#WarningStates; $i++) {
						if ($VoltageState eq $WarningStates[$i]) {
							...
							if ($RequestResult->{$OID_EnvMonVoltageStatusEntry.".3.".$VoltageId}) {#value defined
								$WarningOutput .= "with value = ".$RequestResult->{$OID_EnvMonVoltageStatusEntry.".3.".$VoltageId};
							}
							$CriticalOutput .= ' is in ' . ENVMON_STATES->[$VoltageState] . ' state (' . ENVMON_STATE_DESC->[$VoltageState] . '); ';
							last;
		...
		if (defined($temperature)) {
			...
			foreach (@TemperatureIds) {
				my $TemperatureId= $_;
				my $TemperatureState=$RequestResult->{$OID_EnvMonTemperatureStatusEntry.".6.".$TemperatureId};
				if (!defined($TemperatureState)) {
					$SNMPSession->close();
					$_[1]="Error recovering temperature with id $TemperatureId";
					return UNKNOWN;
				}
				my $TemperatureDesc;
				if (defined($RequestResult->{$OID_EnvMonTemperatureStatusEntry.".2.".$TemperatureId})) {#description defined
					$TemperatureDesc = $RequestResult->{$OID_EnvMonTemperatureStatusEntry.".2.".$TemperatureId};							
				}
				else {
					$SNMPSession->close();
					$_[1]="Error recovering temperature description with id $TemperatureId";
					return UNKNOWN;
				}
				for (my $i=0; $i <= $#CriticalStates; $i++) {
					if ($TemperatureState eq $CriticalStates[$i]) {
						$CriticalOutput .= "Sensor '$TemperatureDesc'";
						if ($RequestResult->{$OID_EnvMonTemperatureStatusEntry.".3.".$TemperatureId}) {#value defined
							$CriticalOutput .= "with value = ".$RequestResult->{$OID_EnvMonTemperatureStatusEntry.".3.".$TemperatureId};
						}
						$CriticalOutput .= ' is in ' . ENVMON_STATES->[$TemperatureState] . ' state (' . ENVMON_STATE_DESC->[$TemperatureState] . '); ';
						last;
					}
				}
				if ($CriticalOutput eq '') {
					for (my $i=0; $i <= $#WarningStates; $i++) {
						if ($TemperatureState eq $WarningStates[$i]) {
							...
							if ($RequestResult->{$OID_EnvMonTemperatureStatusEntry.".3.".$TemperatureId}) {#value defined
								$WarningOutput .= "with value = ".$RequestResult->{$OID_EnvMonTemperatureStatusEntry.".3.".$TemperatureId}."Celsius";
							}
							$WarningOutput .= ' is in ' . ENVMON_STATES->[$TemperatureState] . ' state (' . ENVMON_STATE_DESC->[$TemperatureState] . '); ';
							last;
		...
		if (defined($fan)) {
			...
			foreach (@FanIds) {
				my $FanId= $_;
				my $FanState=$RequestResult->{$OID_EnvMonFanStatusEntry.".3.".$FanId};
				if (!defined($FanState)) {
					$SNMPSession->close();
				...
				my $FanDesc;
				if (defined($RequestResult->{$OID_EnvMonFanStatusEntry.".2.".$FanId})) {#description defined
					$FanDesc = $RequestResult->{$OID_EnvMonFanStatusEntry.".2.".$FanId};							
				}
				else {
					$SNMPSession->close();
				...
				for (my $i=0; $i <= $#CriticalStates; $i++) {
					if ($FanState eq $CriticalStates[$i]) {
						$CriticalOutput .= ' is in ' . ENVMON_STATES->[$FanState] . ' state (' . ENVMON_STATE_DESC->[$FanState] . '); ';
						last;
					}
				}
				if ($CriticalOutput eq '') {
					for (my $i=0; $i <= $#WarningStates; $i++) {
						if ($FanState eq $WarningStates[$i]) {
							$WarningOutput .= ' is in ' . ENVMON_STATES->[$FanState] . ' state (' . ENVMON_STATE_DESC->[$FanState] . '); ';
							last;
						}
					}
				}
		...
		if (defined($supply)) {
			...
			foreach (@SupplyIds) {
				my $SupplyId= $_;
				my $SupplyState=$RequestResult->{$OID_EnvMonSupplyStatusEntry.".3.".$SupplyId};
				...
				my $SupplyDesc ;
				if (defined($RequestResult->{$OID_EnvMonSupplyStatusEntry.".2.".$SupplyId})) {#description defined
					$SupplyDesc = $RequestResult->{$OID_EnvMonSupplyStatusEntry.".2.".$SupplyId};							
				}
				else {
					$SNMPSession->close();
					$_[1]="Error recovering supply description with id $SupplyId";
					return UNKNOWN;
				}
				for (my $i=0; $i <= $#CriticalStates; $i++) {
					if ($SupplyState eq $CriticalStates[$i]) {
						$CriticalOutput .= ' is in ' . ENVMON_STATES->[$SupplyState] . ' state (' . ENVMON_STATE_DESC->[$SupplyState] . '); ';
						last;
					}
				}
				if ($CriticalOutput eq '') {
					for (my $i=0; $i <= $#WarningStates; $i++) {
						if ($SupplyState eq $WarningStates[$i]) {
							$WarningOutput .= ' is in ' . ENVMON_STATES->[$SupplyState] . ' state (' . ENVMON_STATE_DESC->[$SupplyState] . '); ';
							last;
						}
					}
				}
		...
		if ($CriticalOutput ne '') {
			...
			$PluginOutput = "Checked environmental sensor(s) return OK state(s)";
		...
		if ( $#VoltageIds >= 0 || $#TemperatureIds >= 0 ) {
			...
			if ( $#VoltageIds >= 0 ) {
				foreach $VoltageId (@VoltageIds) {
					if (defined($RequestResult->{"$OID_EnvMonVoltageStatusEntry.3.$VoltageId"})) {
				        $VoltageValue=$RequestResult->{"$OID_EnvMonVoltageStatusEntry.3.$VoltageId"};
				        $VoltageValue = $VoltageValue /1000; #Normalice value (mV -> V)
				        if ( defined $RequestResult->{"$OID_EnvMonVoltageStatusEntry.2.$VoltageId"} ) {
				        	$PerformanceData .= " '" . $RequestResult->{"$OID_EnvMonVoltageStatusEntry.2.$VoltageId"} . "'=${TemperatureValue}Celsius;;;" . $RequestResult->{"$OID_EnvMonVoltageStatusEntry.4.$VoltageId"} . ';';
				        }
				        else {
				        	$PerformanceData .= " 'Voltage sensor $VoltageId'=${VoltageValue}V;;;" . $RequestResult->{"$OID_EnvMonVoltageStatusEntry.4.$VoltageId"} . ';';
				        }
					}
				}
			}
			...
			if( $#TemperatureIds >= 0 ) {
				foreach $TemperatureId (@TemperatureIds) {
					if (defined($RequestResult->{"$OID_EnvMonTemperatureStatusEntry.3.$TemperatureId"})) {
				        $TemperatureValue=$RequestResult->{"$OID_EnvMonTemperatureStatusEntry.3.$TemperatureId"};
				        if ( defined $RequestResult->{"$OID_EnvMonTemperatureStatusEntry.2.$TemperatureId"} ) {
				        	$PerformanceData .= " '" . $RequestResult->{"$OID_EnvMonTemperatureStatusEntry.2.$TemperatureId"} . "'=${TemperatureValue}Celsius;;;;" . $RequestResult->{"$OID_EnvMonTemperatureStatusEntry.4.$TemperatureId"};
				        }
				        else {
				        	$PerformanceData .= " 'Temperature sensor $TemperatureId'=$TemperatureValue"."Celsius;;;;" . $RequestResult->{"$OID_EnvMonTemperatureStatusEntry.4.$TemperatureId"};
				        }
					}
				}
			}
